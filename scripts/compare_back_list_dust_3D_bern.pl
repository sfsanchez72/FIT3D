...
if ($#ARGV<13) {
    print "USE: compare_back_list_dust_3D.pl RSS.fits BACK_LIST OUTFILE_FULL OUTFILE_AGEMET MASK_LIST REDSHIFT SIGMA_DISP WAVE_SCALE WAVE_WIDTH FLUX_THRESHOLD PLOT Av_min Av_max delta_Av  [min max] [wmin wmax]\n";
    exit;
}
...
#    $call="compare_back_list.pl spec_junk.txt ".$back_list." comp_back_junk.out ".$mask_list." ".$redshift." ".$sigma." ".$wave_scale." ".$plot;
	$call="compare_back_list_dust.pl spec_junk.txt ".$back_list." comp_back_junk.out ".$mask_list." ".$red_now." ".$sigma." ".$wave_scale." ".$wave_width." ".$plot." ".$Av_min." ".$Av_max." ".$delta_Av;
	#print "$call\n";
	if ($def==1) {
	    $call=$call." ".$min." ".$max;
	}
