...
if ($#ARGV<1) {
    print "USE: spec2D_plot_SDSS.pl INPUT_FILE.FITS NY DEVICE [MIN MAX] [WMIN WMAX]\n [REF_LINES CUT]";
    exit;
}
...
$cdelt=10**(0.0001)*1e-4*log(10);
$COEFF0=$pdl->hdr->{"COEFF0"};
$COEFF1=$pdl->hdr->{"COEFF1"};
...
$pdl_new=interpol(pdl(@wave_new),pdl(@wave),pdl(@flux));

open(FH,">spec_SDSS.out");
for ($i=0;$i<5450;$i++) {
    $val=$pdl_new->at($i);
    print FH "$i $wave_new[$i] $val\n";
}
close(FH);
