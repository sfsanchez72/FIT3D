...
if ($#ARGV<4) {
    print "USE: SII_val_OIII_ratio.pl SII_ratio(VALUE) OIII_ratio OUT_DENSITY OUT_TEMP DEV\n";
    exit;
}
$ratio=$ARGV[0];
$ratio_OIII_file=$ARGV[1];
$out_den=$ARGV[2];
$out_temp=$ARGV[3];
$dev=$ARGV[4];

#$ratio=rfits($ratio_file);
$ratio_OIII=rfits($ratio_OIII_file);
...
$DENSITY=10**($LDENSITY);
$T=3.29e4/(log($ratio_OIII*(1+(4.5e-6)*$DENSITY)/7.73));
$FACT=(10**2)/sqrt($T);
