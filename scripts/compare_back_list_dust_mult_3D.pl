...
if ($#ARGV<12) {
    print "USE: compare_back_list_dust_mult_3D.pl RSS.fits BACK_LIST OUTFILE_FULL OUTFILE_AGEMET MASK_LIST REDSHIFT SIGMA_DISP WAVE_SCALE FLUX_THRESHOLD PLOT Av_min Av_max delta_Av  [min max] [wmin wmax]\n";
    exit;
}
...
$back_list_fits=$back_list.".fits";
$call="spectra2img.pl ".$back_list." ".$back_list_fits;
print "$call\n";
system($call);
...
#
    $call="spec_plot.pl spec_junk.txt /null > spec_plot.out";
    system($call);
...
#    print "PASO $median_flux $flux_limit\n";
	...
	$call="compare_back_list_dust_mult.pl spec_junk.txt ".$back_list_fits." comp_back_junk.out ".$mask_list." ".$red_now." ".$sigma." ".$wave_scale." ".$plot." ".$Av_min." ".$Av_max." ".$delta_Av;
#	print "$call"; <stdin>;
	if ($def==1) {
	    $call=$call." ".$min." ".$max;
	}
