...
if ($#ARGV<13) {
    print "USE: plot_maps_cont.pl map.fits min max bright contrast Label factor dev VAL_TO_WHITE MAP_CONT NLEVELS MIN STEP NWHITE[49] [TABLE REVERSE] [ROT]\n";
    exit;
}
$mapfile=$ARGV[0];
$min=$ARGV[1];
...
$NNx=$nx-1;
$NNy=$ny-1;
$rpdl_map=$pdl_map->slice("$NNx:0,");
@map=list($rpdl_map);
$rpdl_cont=$pdl_cont->slice("$NNx:0,");
@cont=list($rpdl_cont);
$crval1=$h->{CRVAL1};
$crval2=$h->{CRVAL2};
$cdelt1=$h->{CD1_1};
$cdelt2=$h->{CD2_2};
print "$crval1 $crval2 | '$cdelt1' '$cdelt2'\n";
$crval2=$crval2-$cdelt2*$h->{CRPIX2};
$crval1=3600*($crval1-$cdelt1*$h->{CRPIX1})/15;
$cdelt1=$cdelt1*(3600/15)/cos($crval2/180*3.1416);
$cdelt2=3600*$cdelt2;
$crval2=3600*$crval2;
print "CRVAL = $crval1 $cdelt1\n";
...
#@tr=($crval1,$cdelt1,0,$crval2,0,$cdelt2);
#@tr=($x_min,(-1)*$cdelt1,0,$y_max,0,(-1)*$cdelt2);
@tr=($crval1,$cdelt1,0,$crval2,0,$cdelt2);
...
pgsvp(0.15,0.8,0.1,0.9);
pgswin($x_max,$x_min,$y_min,$y_max);
pgtbox("YZN",0,0,"YZNV",0,0);
...
if ($dev !~ "TPNG") { 
  #  pglabel("RA","DEC","");
    pgmtxt("L",3,0.45,0.5,"Declination");
    pgmtxt("BV",3,0.5,0.5,"Right Ascesion");
}
