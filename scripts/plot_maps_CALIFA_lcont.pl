...
if ($#ARGV<13) {
    print "USE: maps_orion.pl map.fits min max bright contrast Label factor dev SYSTEMIC_VEL MAP_CONT NLEVELS MIN STEP NWHITE[49] [ROT] [REVERSE]\n";
    exit;
}
$mapfile=$ARGV[0];
$min=$ARGV[1];
...
for ($j=0;$j<$ny;$j++) {
	...
	if ($cont[$i+$j*$nx]>0) {
	    $lcont[$i+$j*$nx]=log10($cont[$i+$j*$nx]);
	    if ($lcont[$i+$j*$nx]<$min_cont) {
		$map[$i+$j*$nx]=0;
	    }
	} else {
	    $lcont[$i+$j*$nx]=-1000;
	...
	if ($mask[$i+$j*$nx]==0) {
	    if (($i>0)&&($i<$nx-1)) {
		if (($mask[$i-1+$j*$nx]!=0)&&($mask[$i+1+$j*$nx]!=0)) {
		    $mask[$i+$j*$nx]=1;
		}
	    }
	}
...
for ($j=0;$j<=256;$j++) {
    ...
    $F=$F0+(0.9-$F0)*($l[$j])**4;#exp(-((abs(($j-0)/256))**2));    
    $r[$jj] = $F*(($l[$j])**$gamma)+$F1*(-0.1486*$a*cos($t)+1.7828*$a*sin($t));
    $g[$jj] = $F*(($l[$j])**$gamma)+$F1*(-0.29223*$a*cos($t)-0.9065*$a*sin($t));
    
    $b[$jj] = $F*(($l[$j])**$gamma)+$F1*1.97294*$a*cos($t);
...
pgsci(14);
pgslw(10);
#pgcons(\@mask,$nx,$ny,1,$nx,1,$ny,\@levels,2,\@tr);
pgslw(1);
pgsci(1);
...
pgsci(1);
pgslw(4);
pgcons(\@lcont,$nx,$ny,1,$nx,1,$ny,\@levels,$nlevels,\@tr);
pgslw(1);
pgsci(1);
