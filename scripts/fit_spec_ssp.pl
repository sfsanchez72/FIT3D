...
if ($#ARGV<6) {
    print "USE:fit_spec_back.pl SPECTRUM.TXT LINE_FILE START_WAVELENGTH END_WAVELENGTH BACK_LIST NBACK MASK_LIST [CONFIG]\n";    
    exit;
}
...
my @age;
my @met;
$back_age_met="AGE_MET.list";
open(FH,"<$back_list");
open(AM,">$back_age_met");
while($line=<FH>) {
    chop($line);
    if ($line !~ "#") {
	$file[$nf]=$line;
...
if ($no_config==0) {
    ...
    if ($nf>0) {
	$call="fit_spec_ssp fit_spectra.input tmp.config ".$back_age_met." BACK_JUNK.fits ".$nback." ".$device;
    } else {
...
if ($#ARGV==3) {
 ...
 <stdin>;
    ...
    if ($nf>0) {
	$call="fit_spec_ssp fit_spectra.input tmp.config ".$back_age_met." BACK_JUNK.fits ".$nback." ".$device;
#	$call="fit_spec_ssp fit_spectra.input tmp.config BACK_JUNK.fits ".$nback." ".$device;
    } else {
	$call="fit_spec_back fit_spectra.input tmp.config none 0 ".$device;
    }
    system($call);

#$call="fit_spec_back fit_spectra.input tmp.config ".$back_age_met." ".$nback;
#system($call);
print "$call\n";
