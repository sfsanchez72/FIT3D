...
if ($#ARGV<5) {
    print "USE: auto_ssp.pl SPEC1.txt BACK_LIST.fits OUTFILE MASK_LIST CONFIG_FILE PLOT [min max] [wmin wmax] [redshift_elines_to_mask]\n";
    print "CONFIG_FILE:\n";
...
#	print "$i/$n_c\n";
	...
	if ($masked[$i]>0) {
	    $error[$i][$iii]=1/abs($val)+$chi_sec[$i];
