...
while($line=<FH>) {
    ...
    if ($#data>2) {
	$e_flux_unc[$n_unc]=abs($data[3]);
#	if ($e_flux_unc[$n_unc]>sqrt(abs($flux_unc[$n_unc]))) {
...
#    print "PASO por AQUI\n";
    ...
    open(OUT2,">gas_spec.txt");
    for ($j=0;$j<$n_unc;$j++) {
	$res_spec[$j]=$flux_unc[$j];
	if ($color_unc[$j]==1) {
	    print OUT "$index_unc[$j] $wave_unc[$j] $res_spec[$j]\n";
	    print OUT2 "$index_unc[$j] $wave_unc[$j] $res_spec[$j]\n";
	}
    }
    close(OU2);
    close(OUT);
    ...
    for ($j=0;$j<$n_unc;$j++) {
	if ($color_unc[$j]==1) {
	    print OUT_MOD "$index_unc[$j] $wave_unc[$j] 0\n";
	    print OUT_RES "$index_unc[$j] $wave_unc[$j] $res_spec[$j]\n";
	}
    }
    close(OUT_RES);
    close(OUT_MOD);
...
#		$SYS_VEL=$vel_light*$red_elines;
		$SYS_VEL_MAX=$vel_light*$REDSHIFT+$D_SYS_VEL;
		$SYS_VEL_MIN=$vel_light*$REDSHIFT-$D_SYS_VEL;
#		$SYS_VEL_MAX=$vel_light*$red_elines+0.5*$D_SYS_VEL;
#		$SYS_VEL_MIN=$vel_light*$red_elines-0.5*$D_SYS_VEL;
	    }
	}
...
#	$SYS_VEL_MIN=$vel_light*$redshift+10;
	...
	while($line=<IN_CONF>) {
		...
		print  "LINE: $NITER $SYS_VEL   1   $SYS_VEL_MIN  $SYS_VEL_MAX  -1\n";	    
	    } else {
		print OUT_CONF "$line\n";
	    }
	    $n_conf++;
...
# Low order polynomical!
	...
	$call="cp new_res.txt sub_spec.txt";
	system($call);
...
########### WITH VARIANCE IT IS NOT NEEDED #################
if ($have_error==0) {
...
#    print "FIT PASSED $redshift $sigma @Av $i $j $n_unc $nf\n";
