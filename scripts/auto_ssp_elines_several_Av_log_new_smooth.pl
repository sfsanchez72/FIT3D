...
#	}  while ($RED<$max_redshift);
	...
	my $s_x;
	my $coeff;
	($s_x,$coeff)=fitpoly1d(pdl(@red_r),pdl(@chi_r),3);
	$RED=$min_redshift;
	$nk=0;
	$found=0;
	do {
	    ...
	    for ($k=0;$k<3;$k++) {
		$c=$coeff->at($k);
		$out_red[$nk]=$out_red[$nk]+$c*(($RED)**$k);
	    }
#	    print "$red_now[$nk]"

#	    print "$nk $redshift $red_now[$nk] | $out_red[$nk-2] $out_red[$nk-1] $out_red[$nk]\n";
	    if (($nk>1)&&($out_red[$nk-1]<$out_red[$nk-2])&&($out_red[$nk-1]<$out_red[$nk])) {
		$redshift=$red_now[$nk-1];	
		$min_chi_sq=$out_spec[$nk-1];	
		$RED=$max_redshift;
		$found=1;
	    }
	    $nk++;
...
#	    }
	...
	my $s_x;
	my $coeff;
	($s_x,$coeff)=fitpoly1d(pdl(@sigma_s),pdl(@chi_s),4);
	$SIGMA=$min_sigma;
	$nk=0;
	$found=0;
	do {
	    ...
	    for ($k=0;$k<4;$k++) {
		$c=$coeff->at($k);
		$out_sigma[$nk]=$out_sigma[$nk]+$c*(($SIGMA)**$k);
	    }

	    if (($nk>1)&&($out_sigma[$nk-1]<$out_sigma[$nk-2])&&($out_sigma[$nk-1]<$out_sigma[$nk])) {
		$sigma=$sigma_now[$nk-1];	
		$min_chi_sq=$out_spec[$nk-1];	
		$SIGMA=$max_sigma;
		$found=1;
	    }
...
#$call="smooth_spec_clip.pl res_spec.txt tmp_spec.txt ".$NBOX." /null 2  0 
#print "$call\n";
system($call);
...
#    $pdl_flux_c = $pdl_flux_c_conv->slice(",$iii");
    $pdl_flux_c = $pdl_flux_c_ini->slice(",$iii");
...
#	print "$i/$n_c\n";
	...
	$val=0;
	$j_mean=int($n_unc/2);
#	print "$i $j_min $j_max $wave_unc[$i";
	for ($j=$j_min;$j<$j_max;$j++) {
	    $rsigma_now=($wave_unc[$i]/$wave_unc[$j_mean])*$rsigma;
	    $gaus=exp(-0.5*((($j-$i)/$rsigma_now)**2));    
	    $val_now=$out_spec_pdl->at($j);
	    $val=$val+$val_now*$gaus;
	    $norm=$norm+$gaus;
	}
	if ($norm!=0) {
	    $val=$val/$norm;
...
#	print "$wave_unc[$i] $i $j_min $j_max $box $val $norm $rsigma $rsigma_now\n";
#	<stdin>;
#	$kernel=$kernel/$norm;
...
#    print "DONE ( $coeffs)\n"; <stdin>;
    ...
    for ($j=0;$j<$n_unc;$j++) {
	$out_spec_tmp[$j]=($yfit->at($j,0));#
    }
    @med_out_spec_tmp=median_filter(int(10*$rsigma),\@out_spec_tmp);
    @med_flux_unc=median_filter(int(10*$rsigma),\@flux_unc);
    for ($j=0;$j<$n_unc;$j++) {
	if ($med_out_spec_tmp[$j]>0) {
	    $rat_now=$med_flux_unc[$j]/$med_out_spec_tmp[$j];
	} else {
	    $rat_now=0;
	}
	$rat[$j]=$rat_now;
#	$out_spec[$j]=($out_spec_tmp[$j])*$rat_now;#
	$out_spec[$j]=($out_spec_tmp[$j])*$rat_now;#
    }
...
#		$chi=$chi+$masked[$j]*(($flux_unc[$j]-$out_spec[$j])**2)/abs(1.5*$out_spec[$j]);
		$chi=$chi+$masked[$j]*(($flux_masked[$j]-$out_spec_tmp[$j])**2)/abs(1.5*$out_spec[$j]);
...
#    print "PASO $chi_sq $MIN_CHISQ\n";
	...
	for ($j=0;$j<$n_unc;$j++) {
		...
		$model_spec[$j]=$model_spec[$j]*$rat[$j];
...
#	print "$i $j_min $j_max $wave_unc[$i";
		...
		$rsigma_now=($wave_unc[$j]/$wave_unc[$j_mean])*$rsigma;
		$gaus=exp(-0.5*((($jj-$j)/$rsigma_now)**2));    
		$val_now=$model_now->at($jj,0);
		$val=$val+$val_now*$gaus;
		$norm=$norm+$gaus;
	    }
	    if ($norm!=0) {
