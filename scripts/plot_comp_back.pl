...
if ($#ARGV<3) {
    print "USE: plot_comp_back.pl compare_back_out DEVICE SHAPE SIZE [MARK] [MIN MAX] [BRIGHT CONTRAST LUT_TABLE INVERSE(0/1)]\n";
    exit;
}
...
if ($#ARGV>=6) {
    $flux_min_plot=$ARGV[5];
    $flux_max_plot=$ARGV[6];
} else {
...
pgenv($x_max,$x_min,$y_min,$y_max,0,0);
...
for ($i=0;$i<$ns;$i++) {
	...
	if ($shape eq "R") {
	    ...
	    pgsfs(1);
	    if ($flux[$i]>$flux_min_plot) {
		pgrect($x[$i]-$sizex/2,$x[$i]+$sizex/2,$y[$i]-$size/2,$y[$i]+$size/2);
	    }
	    pgsfs(2);
	    pgsci(1);
