...
if ($#ARGV<6) {
    print "USE: get_slice_narrow_image.pl INPUT_IMAGE.fits POS_TABLE.txt PIX_SCALE DELTA_X_PIX DELTA_Y_PIX output_slice.txt FACTOR [DX DY]\n";
    exit;
}
...
#$rat=$factor*(3*1e5)*$PHOTFLAM*$FWHM/($CW**2);
$rat=$factor*$PHOTFLAM*$FWHM/($CW**2)/(3*1e5);
