...
if ($#ARGV<7) {
    print "USE:fit_spec_back_var.pl SPECTRUM.TXT VARIANCE.TXT LINE_FILE START_WAVELENGTH END_WAVELENGTH BACK_LIST NBACK MASK_LIST [CONFIG]\n";    
    exit;
}
...
if ($no_config==0) {
    ...
    if ($nf>0) {
	$call="fit_spec_back_fits_var fit_spectra.input fit_variance.input tmp.config BACK_JUNK.fits ".$nback." ".$device;
    } else {
	$call="fit_spec_back_var fit_spectra.input fit_variance.input tmp.config none 0 ".$device;
    }
    system($call);
...
print "For re-running the program, use:\n fit_spec_back_var fit_spectra.input fit_variance.input out_config.fit_spectra $back_list $nback\n"; 
