...
#    $limit=0.5;
    do {
	$call="peak_find tjunk.fits 0 1 1 5 1 1 ".$limit." tjunk.peaks";
	print "$call\n";
    print LOG "$call\n";
	system($call);
    ...
    print LOG "$call\n";
    system($call);
    $call="trace_peaks_recursive tjunk.fits 0 tjunk.peaks 1 2 0 5 1 tjunk.trc.fits 0 2.5";
    print "$call\n";
    print LOG "$call\n";
    system($call);
...
if ($arc ne "") {
    ...
    print LOG "$call\n";
	    system($call);    
	    if ($type==0) {
		$call="mdist_cor_sp arc.dc0.fits 2 30 ".$order." arc.dc1.fits arc.dist.fits 22 10100 1 0 0 2";
		print "$call\n";
    print LOG "$call\n";
		system($call);    
	    } else {
		$call="mdist_cor_sp arc.dc0.fits 2 30 ".$order." arc.dc1.fits arc.dist.fits 22 10100 1";
		print "$call\n";
    print LOG "$call\n";
		system($call);    
	    }
    ...
    print LOG "$call\n";
		system($call);    
		if ($type==0) {
		    $call="disp_cor arc.dc1.fits ".$crval." ".$cdelt." 2 180 30 ".$order." arc.disp_cor.fits arc.disp.txt 1";
		    print "$call\n";
    print LOG "$call\n";
		    system($call);    
		} else {
		    $call="disp_cor arc.dc1.fits ".$crval." ".$cdelt." 2 128 80 ".$order." arc.disp_cor.fits arc.disp.txt 1";
		    print "$call\n";
    print LOG "$call\n";
		    system($call);    
		}
