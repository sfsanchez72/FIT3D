...
if ($#ARGV<6) {
    print "USE: fit_line.pl SPECTRUM.TXT LINE_FILE START_WAVELENGTH END_WAVELENGTH BACK0 BACK1 BACK2 [CONFIG]\n";    
    exit;
}
...
#		    $tmp=<stdin>;
#		    chop($tmp);
		    ($flux[$nl[$i]][$i],$min_flux[$nl[$i]][$i],$max_flux[$nl[$i]][$i])=(1e6,0,1e20);
		    $nl[$i]++;
		    $n_line++;	
		   # print "Included\n";
	    }	
...
#	print FH "$vel\t 1\t 0\t 0\t $fl_s[$i]\n"; 
    ...
    print FH "poly1d\n";
    print FH "$back0\t 1\t -1e20\t 1e20\t -1\n"; 
    print FH "$back1\t 1\t -1e20\t 1e20\t -1\n"; 
    print FH "$back2\t 0\t -1e20\t 1e20\t -1\n"; 
    for ($k=0;$k<5;$k++) {
	print FH "0\t 0\t 0\t 0\t -1\n";
    }
