...
if ($#ARGV<1) {
    print "USE: fiber_flat.pl SKYFLAT.fits FIBERFLAT.fits [LIMIT MASK]\n";
    exit;
}
...
$pdl=rfits($infile);
($nx,$ny)=$pdl->dims();
$out_pdl=zeroes(float,$nx,$ny);
$mask_pdl=zeroes(float,$nx,$ny);
