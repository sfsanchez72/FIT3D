...
if ($#ARGV<10) {
    print "USE: compare_back_list.pl SPEC1.txt BACK_LIST OUTFILE MASK_LIST REDSHIFT SIGMA_DISP WAVE_SCALE PLOT Av_min Av_max delta_Av [min max] [wmin wmax]\n";
    exit;
}
...
#	my $out_spec_pdl = interpol(pdl(@wave_unc), pdl(@wave_c), pdl(@flux_c));
	...
	$scale=$flux_unc[$i_scale]/($out_spec_pdl->at($i_scale));#*(10**(0.4*$Av*A_l(3.1,$wave_scale)));		
...
#	print "$j $wave_unc[$j] $flux_unc[$j] $out_spec[$j] $chi\n";
	    if ($flux_unc[$j]!=0) {
