...
if ($#ARGV<9) {
    print "USE:fit_single_back.pl SPECTRUM.TXT START_WAVELENGTH END_WAVELENGTH BACK_LIST NBACK MASK_LIST WHICH_BACK[=-1 ALL] REDSHIFT VEL_DISP FILE_OUT [ELINE_CONFIG]\n";    
    exit;
}
...
#
# We create a FITS copy of the backgrouds
#
$nf=0;
open(FH,"<$back_list");
while($line=<FH>) {
...
#		print "$which_back $nnback $w_back $f_back $f_med $ratio\n";
    ...
    open(FH,">tmp.config");
    if ($dz>0) {
	print FH "0 1 0.2 0.001\n";
    } else {
	print FH "0 1 0.002 0.00001\n";
    }
    print FH "back\n";
...
#	$call="fit_spec_back fit_spectra.input tmp.config ".$back_list." ".$nback;
	...
	print "$name[$i] $chisq \n";
	print OUT "$name[$i] $chisq $flux $eflux $disp $edisp $vel $evel\n";		
    }
    close(OUT);
}
