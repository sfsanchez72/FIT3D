...
if ($#ARGV<5) {
    print "USE: dist_cor_external.pl EXTRACTED.fits CORRECTED.fits DISTORSION_CORRECTION.txt start_index delta_index plot\n";
    exit;
}
...
while($line=<FH>) {
    ...
    $index_all[$n]=$data[0];
    $shift_all[$n]=$data[1];
    if ($shift_min>$shift_all[$n]) {
	$shift_min=$shift_all[$n];
    }
    if ($shift_max<$shift_all[$n]) {
	$shift_max=$shift_all[$n];
    }
    $n++;
}
close(FH);
