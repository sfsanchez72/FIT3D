...
if ($#ARGV<7) {
    print "USE: radial_mean_cube_ring.pl CUBE.fits Delta_R X_C Y_C OUTPUT.RSS.fits WMIN WMAX NSIGMA [PLOT]\n";
...
#
	...
	if (($r[$j]>=$r_min)&&($r[$j]<$r_max)) {
		...
		if (($wave[$iii]>$Wmin)&&($wave[$iii]<$Wmax)) {
		    $val=$slice->at($iii);
		    $val_t=$t->at($iii);
		    $val_tt=$tt->at($iii);
		    $chi=$chi+($val-$val_t)**2;
		    $sigma=$sigma+$val_tt**2;
		}
	    }
	    $rat=$chi/$sigma;
...
#    print "$rat $chi $sigma $nsig $nsum2\n";
    $T.=$T/$nsum2;
...
$pdl_e_out->hdr->{CRPIX1}=$crpix; 
$pdl_e_out->hdr->{CRVAL1}=$crval; 
$pdl_e_out->hdr->{CDELT1}=$cdelt; 
$pdl_e_out->wfits($e_output);
