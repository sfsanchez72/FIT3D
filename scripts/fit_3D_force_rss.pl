...
if ($#ARGV<4) {
    print "USE: fit_3D_force.pl RSS_FITSFILE PT_FILE CONFIG_FILE INPUT_FILE OUT_FILE [FRE_FRACT] [BIAS] [th]\n";    
    exit;
}
...
#    for ($i=0;$i<$n_mod;$i++) {
	...
	for ($j=0;$j<9;$j++) {
	    $a[$i][$j]=$data[1+2*$j];
#	    print "$i $a[$i][$j]\n";
	    if (($ia[$i][$j]==1)&&($flag[$i][$j]==-1)&&($free_frac_def==1)&&($free_frac>0)) {
...
#	    print "$a[$i][$j] $ia[$i][$j] $a_m[$i][$j] $a_M[$i][$j] $flag[$i][$j]\n";
...
system("rm mod_fit_3D_force.fits");
write_rss("mod_fit_3D_force.fits",$npix,$nb_spec,$start_w,$delta_w,\@a_mod);
print "MODEL=mod_fit_3D_force.fits\n";
system("rm res_fit_3D_force.fits");
write_rss("res_fit_3D_force.fits",$npix,$nb_spec,$start_w,$delta_w,\@a_res);
print "RES=res_fit_3D_force.fits\n";
