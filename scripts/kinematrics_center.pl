...
if ($#ARGV<15) {
    print "USE: kinematrics_center.pl map.fits min max bright contrast Label factor dev SYSTEMIC_VEL MAP_CONT NLEVELS MIN STEP NWHITE[49] XC YC [ROT]\n";
    exit;
}
$mapfile=$ARGV[0];
$min=$ARGV[1];
...
for ($r=1;$r<$r_max;$r++) {
	...
	for ($i=$i_min;$i<$i_max;$i++) {
	    $r_now=sqrt(($i-$XC)**2+($j-$YC)**2);
	    if (($r_now>$r-1)&&($r_now<$r+2)) {
		if (($cont[$i+$j*$nx]>$min_cont)&&($map[$i+$j*$nx]>$min)&&($map[$i+$j*$nx]<$max)&&($map[$i+$j*$nx]!=0))  {
...
for ($r=1;$r<$r_max;$r++) {
	...
	for ($i=$i_min;$i<$i_max;$i++) {
		...
		if (($cont[$i+$j*$nx]>$min_cont)&&($map[$i+$j*$nx]>$min)&&($map[$i+$j*$nx]<$max)&&($map[$i+$j*$nx]!=0))  {
		    ...
		    if ($map[$i+$j*$nx]<$vel_a_min) {
			$vel_a_min=$map[$i+$j*$nx];
			$vel_max[$kk]=$vel_a_min;
			$i_vel_max[$kk]=$i;
			$j_vel_max[$kk]=$j;
			$def_max=1;
		    }
...
pgsci(3);
pgslw(5);
pgpoint($kk,\@i_vel_max,\@j_vel_max,3);
pgsci(7);
pgslw(5);
pgpoint($k,\@i_vel_min,\@j_vel_min,2);
pgsch(1.2);
pgslw(3);
