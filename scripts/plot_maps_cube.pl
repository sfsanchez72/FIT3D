...
if ($#ARGV<9) {
    print "USE: plot_maps_cube.pl cube.fits min max bright contrast Label factor dev  START_WAVE END_WAVE [MASK_MAP]\n";
    exit;
}
$mapfile=$ARGV[0];
$min=$ARGV[1];
...
$start_i=int(($w_min-$crval3)/$cdelt3);
$end_i=int(($w_max-$crval3)/$cdelt3);

$pdl_map=$b->slice(":,:,$start_i:$end_i");
...
@tr=($x_min,$cdelt1,0,$crval2,0,$cdelt2);
