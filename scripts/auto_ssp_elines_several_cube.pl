...
$NG_cube_file="NG_".$outfile.".fits";

$mod_cube=zeroes($nx,$ny,$nz);
$res_cube=zeroes($nx,$ny,$nz);
$NG_cube=zeroes($nx,$ny,$nz);
...
#    close(SPEC);
    ...
    for ($is=0;$is<$ns;$is++) {
	if ($is==0) {
	    $SYS_VEL_MAX=$vel_light*$redshift-50;
	    $SYS_VEL_MIN=$vel_light*$redshift+50;
	} else {
	    $SYS_VEL_MAX=$vel_light*$redshift-50;
	    $SYS_VEL_MIN=$vel_light*$redshift+50;
	}
...
#	print "CONF=$config_e\n";
	...
	system($call);
#	print "$call\n";
#	print "Press Enter ($dev)\n"; <stdin>;
#	$call="cat tmp_e.config";
#	system($call);
#	print "$call\n";
#	print "DONE $is\n";
#	<stdin>;
