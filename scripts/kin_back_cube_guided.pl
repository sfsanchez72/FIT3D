...
if ($#ARGV<8) {
    print "USE: kin_back_cube.pl cube.fits config_file back_file nback start_wavelength end_wavelength out_file DEVICE GUIDED_VEL_MAP.fits [ASK]\n";
    exit;
}
...
for ($j=0;$j<$ny;$j++) {
	...
	while($ff=<FF>) {
		...
		if ($vel_now!=0) {
		    $vel_now1=$vel_now-50;
		    $vel_now2=$vel_now+50;
		    print OF "$vel_now   1 $vel_now1 $vel_now2 -1\n";
		} else {
		    print OF "$ff\n";
		}
