...
if ($#ARGV<5) {
    print "USE: table_plot_csv.pl ASCII_TABLE.txt N.COLUMN1(0...N)  N.COLUMN2(0...N) LABEL1 LABEL2 DEV [MIN MAX] [YMIN YMAX] [FIX] [ONE-to-ONE_LINE 0/1]\n";
    exit;
}
...
$pdl_yy=pdl(@y);
$index=qsorti($pdl_xx);
$q_pdl_xx=$pdl_xx->index($index);
$q_pdl_yy=$pdl_yy->index($index);
($s_x,$coeff) = fitpoly1d($q_pdl_xx,$q_pdl_yy,2);
print "REG $coeff\n";
#@REG=regression(\@x,\@y);
