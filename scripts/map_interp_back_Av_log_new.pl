...
#print "GRID=$ns $x_min $x_max $y_min $y_max $dpix [$nx,$ny]\n";
...
#		print "$line | $n_row | ZERO \n";
    ...
    for ($i=$n_mod_old;$i<($n_mod+$n_mod_old);$i++) {
#	print "$i\n";
	if ($i>$n_mod_max) {
	    $n_mod_max=$i;
	}
	$line=<FH>; #print "$n_row,$n_mod,$n_mod_old $i\n";
	chop($line);
	if ($line =~ "eline") {
	    @data=split(" ",$line);
	    if (($n_row>-1)&&($i>-1)) {
...
#	print "AV=$AV[$n_row][$i] $e_AV[$n_row][$i]\n";
...
for ($i=0;$i<$n_mod;$i++) {
	...
	$slice_f=$prefix."_flux_".$num.".slice";
...
#
	...
	system($call);
	$call="cp junk_int.tmp ".$prefix."_flux_".$num.".txt";
	system($call);
	$call="cp junk_int.tmp ".$slice_f;
	system($call);
	...
	open(JUNK,">junk_int.tmp");
	for ($j=0;$j<$n_row;$j++) {
	    printf JUNK "$j $x[$j] $y[$j] $AV[$j][$i]\n";
	}
	close(JUNK);
	system("rm $outfile_AV");
	$call="interpol -if junk_int.tmp -of ".$outfile_AV." -dp ".$int_dpix." -gf ".$int_mode." -go ".$int_opt." -nc 4";
	system($call);

	open(JUNK,">junk_int.tmp");
	for ($j=0;$j<$n_row;$j++) {
	    printf JUNK "$j $x[$j] $y[$j] $e_AV[$j][$i]\n";
	}
	close(JUNK);
	system("rm $outfile_eAV");
	$call="interpol -if junk_int.tmp -of ".$outfile_eAV." -dp ".$int_dpix." -gf ".$int_mode." -go ".$int_opt." -nc 4";
	system($call);
...
$outfile=$prefix."_FLUX.slice";
open(JUNK,">$outfile");
print JUNK "# (1) ID\n";
print JUNK "# (2) X_CENTROID\n";
print JUNK "# (3) Y_CENTROID\n";
for ($i=0;$i<$n_mod;$i++) {
    $k=2*$i+4;
    print JUNK "# ($k) FLUX_$w[0][$i]\n";
    $k=2*$i+5;
    print JUNK "# ($k) ERROR_FLUX_$w[0][$i]\n";
}
#print "PASO $n_row\n";
for ($j=0;$j<$n_row;$j++) {
    $k=$j+1;
	printf JUNK ("%3d %5.2f %5.2f",$k,$x[$j],$y[$j]);
	for ($i=0;$i<$n_mod;$i++) {
	    printf JUNK (" %8.2f %8.2f",$f[$j][$i],$e_f[$j][$i]);
	}
	print JUNK " \n";
}
close(JUNK);

$outfile=$prefix."_VEL.slice";
open(JUNK,">$outfile");
print JUNK "# (1) ID\n";
print JUNK "# (2) X_CENTROID\n";
print JUNK "# (3) Y_CENTROID\n";
for ($i=0;$i<$n_mod;$i++) {
    $k=2*$i+4;
    print JUNK "# ($k) VELOCITY_$w[0][$i] (km/s)\n";
    $k=2*$i+5;
    print JUNK "# ($k) ERROR_VELOCITY_$w[0][$i]\n";
}
for ($j=0;$j<$n_row;$j++) {
#	printf JUNK ("%3d %5.2f %5.2f",$j,$x[$j],$y[$j]);
    $k=$j+1;
	printf JUNK ("%3d %5.2f %5.2f",$k,$x[$j],$y[$j]);
	for ($i=0;$i<$n_mod;$i++) {
	    printf JUNK (" %8.2f %8.2f",$v[$j][$i],$e_v[$j][$i]);
	}
	print JUNK " \n";
}
close(JUNK);

$outfile=$prefix."_DISP.slice";
open(JUNK,">$outfile");
print JUNK "# (1) ID\n";
print JUNK "# (2) X_CENTROID\n";
print JUNK "# (3) Y_CENTROID\n";
for ($i=0;$i<$n_mod;$i++) {
    $k=2*$i+4;
    print JUNK "# ($k) DISP_$w[0][$i] (AA)\n";
    $k=2*$i+5;
    print JUNK "# ($k) ERROR_DISP_$w[0][$i]\n";
}
for ($j=0;$j<$n_row;$j++) {
    $k=$j+1;
	printf JUNK ("%3d %5.2f %5.2f",$k,$x[$j],$y[$j]);
#	printf JUNK ("%3d %5.2f %5.2f",$j,$x[$j],$y[$j]);
	for ($i=0;$i<$n_mod;$i++) {
	    printf JUNK (" %8.2f %8.2f",$dw[$j][$i],$e_dw[$j][$i]);
	}
	print JUNK " \n";
}
close(JUNK);
