...
if ($#ARGV<10) {
    print "USE: compare_back_list_mult.pl SPEC1.txt BACK_LIST.fits OUTFILE MASK_LIST REDSHIFT SIGMA_DISP WAVE_SCALE PLOT Av_min Av_max delta_Av [min max] [wmin wmax]\n";
    exit;
}
...
#
	...
	$pdl_error_new=zeroes($n_unc,$nf_new);
	$nf_i=0;
	for ($k=0;$k<$nf;$k++) {
	    $C=$coeffs->at($k,0);
	    if ($C>0) {
