...
if ($#ARGV<11) {
    print "USE: calib_det.pl UNCALIBRATED_SPEC.txt CALIBRATED_SPEC.txt ratio.txt FACTOR EXTINC_AV AIRMASS SMOOTH_BOX1 SMOOTH2_BOX NPOLY plot min_wave max_wave\n";
    exit;
}
$unc_file=$ARGV[0];
$c_file=$ARGV[1];
...
while($line=<FH>) {
	...
	$flux_unc_ini[$n_unc]=$data[2]*$factor/($ext);
	$n_unc++;
    }
}
close(FH);
...
for ($i=0;$i<$n_unc;$i++) {
    ...
    if (($wave_unc[$i]>$min_wave)&&($wave_unc[$i]<$max_wave)) {
	$w_sec[$n_sec]=$x_ratio[$i];
	$rat_sec[$n_sec]=$y_ratio[$i];
	$n_sec++;
    }
}

($s_x,$coeff) = fitpoly1d(pdl(@w_sec),pdl(@rat_sec),$npoly);
@c=list($coeff);
for ($i=0;$i<$n_unc;$i++) {
    $out_rat[$i]=0;
    for ($k=0;$k<$npoly;$k++) {
	$out_rat[$i]=$out_rat[$i]+$c[$k]*(($x_ratio[$i])**$k);
    }    
#    print "$i $x_ratio[$i] $y_ratio[$i] $out_rat[$i]\n";
...
#    pgline($n_unc,\@wave_unc,\@s_ratio);       
    ...
    pgsci(5);
#    pgline($n_unc,\@wave_unc,\@s_ratio);       
    pgline($#x_ratio,\@x_ratio,\@out_rat);       
#    pgsci(5);
#    pgline($n_unc,\@wave_unc,\@s_ratio);       
...
#    print FH "$j $wave_unc[$j] $y_ratio[$j]\n";
    print FH "$j $wave_unc[$j] $out_rat[$j]\n";
}
close(FH);
