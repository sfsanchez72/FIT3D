...
if ($#ARGV<4) {
    print "USE: create_sky_ppak.pl INPUT OBJECT SKY npoly PLOT\n";
    exit;
}
...
#	print "$j $pos[$j] $array[$j]\n";
    }

    @marray=median_clip1d(\@array,36,8,1);
#    @marray=median_filter(10,\@array);
#    print "$#array $#marray\n";
    ($s_f,$coeff) = fitpoly1d(pdl(@pos),pdl(@marray),$npoly);
#    my $pdl_array = interpol(pdl(@new_pos), pdl(@pos), pdl(@array));
