...
if ($#ARGV<9) {
    print "USE: mdist_cor_ppak.pl EXTRACTED_SCI.fits EXTRACTED_CAL.fits APERTURE NSIGMA NPOLY OUTPUT.fits DISTORSION.fits NX_min NX_max plot [APERTURE2] [NPOLY2] [NPOLY3] [INIT_SHIFT]\n";
    exit;
}
...
#	($s_f,$coeff) = fitpoly1d(pdl(@yp),pdl(@xp_cal),11);
#	print "PASO 1\n";
	for ($j=0;$j<$ny;$j++) {	
...
#	print "PASO 2\n";
	($s_f,$coeff2) = fitpoly1d(pdl(@yp),pdl(@delta_xp),$npoly3);
	for ($j=0;$j<$ny;$j++) {	
	    $a_delta_peaks[$k][$j]=0;
	    for ($kp=0;$kp<$npoly3;$kp++) {
		$c=$coeff2->at($kp);
		$a_delta_peaks[$k][$j]=$a_delta_peaks[$k][$j]+$c*($j**$kp);
#		print "$kp $c $k,$j $a_delta_peaks[$k][$j]\n";
	    }	    
